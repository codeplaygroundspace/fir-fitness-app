...
export function BodySectionFilter({
  ...
  const isBodySection = (category: string) => {
    return bodySections.some(section => section.toLowerCase() === category.toLowerCase())
  }

  // Identify body region categories
  const bodyCategories = categories.filter(
    category => isBodySection(category) || (!category.startsWith('FIR:') && category)
  )
  ...
  const sortedBodyCategories = [...bodyCategories].sort((a, b) => {
    ...
    const aIndex = bodySections.findIndex(section => section.toLowerCase() === a.toLowerCase())
    const bIndex = bodySections.findIndex(section => section.toLowerCase() === b.toLowerCase())
