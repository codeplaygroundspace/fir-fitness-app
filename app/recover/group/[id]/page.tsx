...
export default async function RecoverGroupPage({ params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [groupResponse, exercisesResponse] = await Promise.all([
      ...
      fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/exercises?group=${groupId}&category=recover`,
        {
          cache: 'no-store',
          next: { revalidate: 3600 },
        }
        ...
        <div className="px-4">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold">
              {capitalizeFirstLetter(groupData?.name || `Exercise Group ${groupId}`)} - Recovery
            </h1>
          </div>
          ...
          {exercises.length > 0 ? (
            ...
            <div className="text-center py-8">
              <Alert className="max-w-md mx-auto">
                <AlertTitle>No recovery exercises found</AlertTitle>
                <AlertDescription>
                  There are no recovery exercises assigned to this muscle group yet.
                </AlertDescription>
              </Alert>
              <div className="mt-6 flex flex-col items-center gap-4">
                <p className="text-muted-foreground">
                  No stretches or recovery exercises are available for this muscle group.
                </p>
                <Button asChild variant="outline">
                  <a href="/recover">Return to Recover page</a>
                </Button>
              </div>
            </div>
          )}
    ...
    console.error('Error in RecoverGroupPage:', error)
    return notFound()
  }
}